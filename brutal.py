...
{ungu}   .'|  |  |`. {W}[{Y}•{W}] Info Creator{R} :{W} Ery Wibu
{ungu}  (  |  |  |  )
...
def wa():
	...
	Subrek=requests.post("https://evermos.com/api/register/phone-registration",headers=heading,data=ammarganz).text
	Subrekin=requests.post("https://evermos.com/api/register/phone-registration",headers=heading,data=ammarganz).text
